...
def augustus_functional():
    ...
    cmd = ['augustus', '--species=anidulans',
           '--softmasking=1', '--gff3=on', '--UTR=off',
           '--proteinprofile={}'.format(os.path.join(datadir, 'EOG092C0B3U.prfl')),
           '--stopCodonExcludedFromCDS=False',
           os.path.join(datadir, 'busco_test.fa')]
    functional = False
...
def proteinprofile(fasta, prfl, species='anidulans',
                   start=False, end=False, configpath=False):
    if not configpath:
        try:
