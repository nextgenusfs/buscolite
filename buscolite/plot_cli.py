...
from .plot import BuscoPlot, generate_multi_plot
    ...
    for input_file in args.input:
        try:
            ...
            if "stats" in data and "lineage" in data:
                ...
                print(
                    f"Error: {input_file} appears to be in old format (missing stats/lineage).",
                    file=sys.stderr,
                )
                print(
                    "Please re-run buscolite analysis to generate the new JSON format.",
                    file=sys.stderr,
                )
                sys.exit(1)
